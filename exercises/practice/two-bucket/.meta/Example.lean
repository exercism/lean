...
partial def simulate (goal : Volume) (transforms : List Transform) (constraint : Constraint) (seen : HashSet Volumes) (queue : Queue State) : Option Result :=
  match queue.dequeue? with
  | none               => none
  | some (state, tail) =>
      if state.volumes ∈ seen then
        simulate goal transforms constraint seen tail
      else
        if state.volumes.one = goal then
          some ⟨state.moves, .one, state.volumes.two⟩
        else if state.volumes.two = goal then
          some ⟨state.moves, .two, state.volumes.one⟩
        else
          let nextSeen := seen.insert state.volumes
          let nextStates := transforms.map (· state) |>.filter (λ s => s.volumes ∉ nextSeen ∧ constraint s)
          simulate goal transforms constraint nextSeen (tail.enqueueAll nextStates)
...
def measure (one two : Capacity) (goal : Volume) (start : BucketId) : Option Result :=
  if goal > one ∧ goal > two then none
    ...
    simulate goal transforms constraint {} (Queue.enqueue startState .empty)
